...
func Validate(cfg, old *Config) error {
	...
	if err := cfg.validateCIDRs(cfg.SSHAllowList()); err != nil {
		return errors.Trace(err)
	}

	if err := cfg.validateCIDRs(cfg.ApplicationOfferIngressAllowList()); err != nil {
		return errors.Trace(err)
	}
...
func (c *Config) validateCIDRs(cidrs []string) error {
	for _, cidr := range cidrs {
