...
func (s *stateSuite) TestAllProvisionedControllerReadyTrue(c *gc.C) {
	...
	_, _, err = st.EnsureUpgradeInfo(context.Background(), "0", version.MustParse("3.0.0"), version.MustParse("3.0.1"))
	c.Assert(err, jc.ErrorIsNil)
	_, _, err = st.EnsureUpgradeInfo(context.Background(), "1", version.MustParse("3.0.0"), version.MustParse("3.0.1"))
	c.Assert(err, jc.ErrorIsNil)

	allProvisioned, err := st.AllProvisionedControllersReady(context.Background())
	c.Assert(err, jc.ErrorIsNil)
...
func (s *stateSuite) TestAllProvisionedControllerReadyFalse(c *gc.C) {
	...
	_, err := db.Exec("INSERT INTO controller_node (controller_id) VALUES ('1'), ('2')")
	c.Assert(err, jc.ErrorIsNil)
...
func (s *stateSuite) TestStartUpgrade(c *gc.C) {
	...
	err = st.StartUpgrade(context.Background())

	info, _, err := st.EnsureUpgradeInfo(context.Background(), "0", version.MustParse("3.0.0"), version.MustParse("3.0.1"))
	c.Assert(err, jc.ErrorIsNil)
...
func (s *stateSuite) TestStartUpgradeBeforeCreated(c *gc.C) {
	...
	err := st.StartUpgrade(context.Background())
	c.Assert(err, gc.ErrorMatches, "no current upgrade")
}
