...
func (s *serviceSuite) TestEnsureUpgradeInfo(c *gc.C) {
	...
	s.state.EXPECT().EnsureUpgradeInfo(gomock.Any(), "1", version.MustParse("3.0.0"), version.MustParse("3.0.1")).Return(uistate.Info{
		...
		TargetVersion:   "3.0.1",
		CreatedAt:       "2023-06-20T15:37:17Z",
		StartedAt:       sql.NullString{String: "2023-06-20T15:37:18Z", Valid: true},
	...
	c.Assert(info, gc.DeepEquals, Info{
		...
		CreatedAt:        mustParseTime("2023-06-20T15:37:17Z"),
		StartedAt:        mustParseTime("2023-06-20T15:37:18Z"),
...
func (s *serviceSuite) TestAllProvisioneddControllerReadyTrue(c *gc.C) {
	...
	s.state.EXPECT().AllProvisionedControllersReady(gomock.Any()).Return(true, nil)

	allProvisioned, err := NewService(s.state).AllProvisionedControllersReady(context.Background())
	c.Assert(err, jc.ErrorIsNil)
