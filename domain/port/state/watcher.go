...
// GetApplicationNamesForEndpoints returns a slice of application names that host
// the provided endpoints.
func (st *State) GetApplicationNamesForEndpoints(ctx context.Context, eps []string) ([]string, error) {
	db, err := st.DB()
	if err != nil {
		return nil, jujuerrors.Trace(err)
	}
