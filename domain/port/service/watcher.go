...
type WatcherState interface {
	...
	// GetApplicationNamesForEndpoints returns a slice of application names that
	// host the provided endpoints.
	GetApplicationNamesForEndpoints(ctx context.Context, eps []string) ([]string, error)
}
...
func (s *WatchableService) endpointToApplicationMapper(
	...
	applicationNames, err := s.st.GetApplicationNamesForEndpoints(ctx, endpointUUIDs)
	if err != nil {
		return nil, err
	}
